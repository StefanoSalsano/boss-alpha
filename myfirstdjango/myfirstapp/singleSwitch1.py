...
def singleSwitch1(request, DpidUrl):
    ...
    listMacP = {}
    listCP = {}
    r_SSwOne = requests.get ( 'http://localhost:8080/wm/core/switch/'+DpidUrl+'/port/json' )    #request One that contains the info about single switch
    ...
    while n<len(listSplit_SSwOne):    #to put in infoSwitch_1 the wanted informations
        ...
        for scroll in range(len(listInfo_SSw)):
            ...
            if x>0:
                ...
                i = i+1
        n = n+1
    r_SSwTwo = requests.get ( 'http://localhost:8080/wm/core/switch/'+DpidUrl+'/features/json' )    #request Two
    ...
    listSplit_SSwTwo = string.split ( ra_SSwTwo[0], "{" )    #list with rest api splitted in each elements
